...
Notes: one needs to register at **Open data platform mobility Switzerland (ODPMS)** https://data.opentransportdata.swiss to get access to actual traffic data from traffic counters. Also, calculations in code are dependent on the step_length parameter, so it needs to be coupled with the parameter step length in .sumocfg to have everything working properly. Use the values for step length recommended at (https://sumo.dlr.de/userdoc/Car-Following-Models/EIDM.html). As well as, so far the EIDM model was not tested with option --step-method.ballistic.  
...
You can send an email with your name and affiliation to kresimir.kusic@fpz.unizg.hr
